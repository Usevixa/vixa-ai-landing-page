...
export default function CoreMessage() {
  ...
  useGSAP(
    () => {
      if (prefersReducedMotion()) {
        gsap.set('.m2-big .m2-word', { color: '#EDEDE4' });
        gsap.set('.m2-small .m2-word', { color: '#9A9A8C' });
        gsap.set('.m2-punch', { color: '#C3E043', opacity: 1, y: 0 });
        return;
      }
      ...
      tl.to('.m2-big .m2-word', { color: '#EDEDE4', stagger: 0.35 / 5, duration: 0.06, ease: 'none' }, 0.06);
      tl.to('.m2-small .m2-word', { color: '#9A9A8C', stagger: 0.28 / 9, duration: 0.05, ease: 'none' }, 0.45);
      // …then the silence (0.73 → 0.86), then the payoff
      tl.to('.m2-punch', { opacity: 1, y: 0, color: '#C3E043', duration: 0.12, ease: 'power2.out' }, 0.86);
    },
    { scope: section },
  );
  ...
  return (
    <section ref={section} aria-label="Core message">
      <div className="m2-pin gutter flex h-screen items-center justify-center overflow-hidden text-center">
        <div className="content-col">
          {/* real text for AT/SEO; the split spans are presentation-only (§9) */}
          <p className="sr-only">
            {BIG} {SMALL} 👉 WhatsApp.
          </p>
          <p className="m2-big font-display text-display-xl mx-auto max-w-[16ch] font-bold text-vx-ash">
            <Words text={BIG} />
          </p>
          <p className="m2-small text-body-lg mx-auto mt-8 max-w-[52ch] text-[#83836F]">
            <Words text={SMALL} />
          </p>
          <p className="m2-punch font-display text-display-lg mt-14 font-bold text-vx-lime" aria-hidden="true">
